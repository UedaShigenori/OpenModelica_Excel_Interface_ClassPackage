...
http://japan-miyabi.com/thermal_light/data/03/conductivity.htm
    ...
    model CoffeeTest11
      ...
      Modelica.Blocks.Sources.Constant const(k = 65 * R ^ 2 * pi) annotation(
        Placement(visible = true, transformation(origin = {-54, 70}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
      CoffeeTemperature.CoffeeThermal.NaturalConvection naturalConvection1(L = 2 * (R + cup_t), area = 2 * (R + cup_t) * pi * L, convectionType = CoffeeTemperature.UnitTest.ConvectionType.vertical_planes_cylinder) annotation(
        Placement(visible = true, transformation(origin = {-142, -74}, extent = {{10, -10}, {-10, 10}}, rotation = 0)));
      CoffeeTemperature.CoffeeThermal.NaturalConvection naturalConvection21(L = (R + cup_t) ^ 2 * pi / ((R + cup_t) * 2 * pi), area = 2 * (R + cup_t) ^ 2 * pi, convectionType = CoffeeTemperature.UnitTest.ConvectionType.LowerSurface_of_heatedPlates) annotation(
        Placement(visible = true, transformation(origin = {-142, -124}, extent = {{10, -10}, {-10, 10}}, rotation = 0)));
